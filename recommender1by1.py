...
class Recommender():
	...
	def __init__(self, df_items, df_reviews, user_item_grouped,
		...
		self.user_item_grouped = user_item_grouped
		...
		self.user_item_grouped = (
			self.df_reviews.groupby([self.user_id_colname,
									 self.item_id_colname])[self.rating_col_name].max()
		)


	def fit(self, _id, latent_features=12, learning_rate=0.001, iters=10):
		"""
		...
		return user_mat_done, item_mat_done
